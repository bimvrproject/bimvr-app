	...
	<div style="position: relative;">
		...
		<div style="margin-top: 50px; margin-left: 1.5%; margin-right: 1.5%; margin-bottom: 49px; position:absolute; width: 97%;" >
			<div>
				...
				<a-row v-for="(item, index) in drwaing" :key="index">
					<a-col :span="24">
						<a-collapse >
							<img src="../../assets/img/file_on.png" style="width: 22px;height: 22px; float: left; margin-top: 10px; margin-left: 20px; margin-right: 10px;" />
		...
		<div style="position: fixed; bottom: 0; width: 100%; height: 49px; background-color: #FAFAFA">
			<a-row>
				...
				<a-col :span="6">
					<div v-on:click="openKnock()"><img src="../../assets/img/project_tar_icon_collision_nor@3x.png" style="width: 22px; height: 22px; margin-top: 5px;" /></div>
					<div><a style="color: #666666; font-size: 11px">碰撞</a></div>
				</a-col>
			</a-row>
		</div>
		<!-- footer结束 -->
...
export default {
	...
	mounted() {
        ...
        .then(res => {
					...
					console.log(this.drwaing[0].name)
        })
        .catch(err => {
