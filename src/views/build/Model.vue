	...
	<div>
		...
		<div id="header_page_bg" style="position: fixed; top: 0; height: 44px; width: 100%;">
			<a-row>
				...
				<a-col :span="18"><h4 style="color: #FFFFFF; font-size: 18px; font-family: PingFangSC-Medium; margin-top: 9px;">模型全景</h4></a-col>
				<a-col :span="3"></a-col>
			</a-row>
		</div>
		<!-- header结束 -->
		...
		<div style="position: fixed; bottom: 0; width: 100%; height: 49px; background-color: #FAFAFA">
			<a-row>
				<a-col :span="8">
					<div v-on:click="openModel()"><img src="../../assets/img/model_on.png" style="width: 22px; height: 22px; margin-top: 5px;" /></div>
...
export default {
	...
	mounted() {
		this.$http.post(api.GetResModel,qs.stringify({ modelId: this.$route.query.modelId, projectId: this.$route.query.projectId, companyId: this.$route.query.companyId }))
        .then(res => {
